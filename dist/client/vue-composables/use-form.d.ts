...
export declare const getZodErrors: <Shape extends Record<string, ZodTypeAny>, Schema extends ZodEffectsUnion<ZodObject<Shape, "strip", ZodTypeAny, { [k_1 in keyof import("zod").objectUtil.addQuestionMarks<{ [k in keyof Shape]: Shape[k]["_output"]; }>]: import("zod").objectUtil.addQuestionMarks<{ [k in keyof Shape]: Shape[k]["_output"]; }>[k_1]; }, { [k_3 in keyof import("zod").objectUtil.addQuestionMarks<{ [k_2 in keyof Shape]: Shape[k_2]["_input"]; }>]: import("zod").objectUtil.addQuestionMarks<{ [k_2 in keyof Shape]: Shape[k_2]["_input"]; }>[k_3]; }>>, ErrorsResult extends Partial<{ [key in keyof ReturnType<Schema["parse"]>]: ReturnType<Schema["parse"]>[key] extends (infer val)[] ? val extends Record<string, unknown> ? Partial<{ [key_1 in keyof val]: val[key_1] extends (infer val)[] ? val extends Record<string, unknown> ? Partial<any>[] : string[] : string[]; }>[] : string[] : string[]; }>>(schema: Schema, form: Record<string, unknown>) => ErrorsResult;
declare const useForm: <Shape extends Record<string, ZodTypeAny>, Schema extends ZodEffectsUnion<ZodObject<Shape, "strip", ZodTypeAny, { [k_1 in keyof import("zod").objectUtil.addQuestionMarks<{ [k in keyof Shape]: Shape[k]["_output"]; }>]: import("zod").objectUtil.addQuestionMarks<{ [k in keyof Shape]: Shape[k]["_output"]; }>[k_1]; }, { [k_3 in keyof import("zod").objectUtil.addQuestionMarks<{ [k_2 in keyof Shape]: Shape[k_2]["_input"]; }>]: import("zod").objectUtil.addQuestionMarks<{ [k_2 in keyof Shape]: Shape[k_2]["_input"]; }>[k_3]; }>>, ValidForm extends ReturnType<Schema["parse"]>, Form extends Nullable<ValidForm>>(schema: Schema, defaultForm: Form, processing: Ref<boolean> | ComputedRef<boolean>, onSubmit: (form: ValidForm) => Promise<void>, clearAfterSubmit?: boolean) => {
    ...
    errors: ComputedRef<Partial<{ [key in keyof ReturnType<Schema["parse"]>]: ReturnType<Schema["parse"]>[key] extends (infer val)[] ? val extends Record<string, unknown> ? Partial<{ [key_1 in keyof val]: val[key_1] extends (infer val)[] ? val extends Record<string, unknown> ? Partial<any>[] : string[] : string[]; }>[] : string[] : string[]; }>>;
